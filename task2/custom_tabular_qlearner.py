...
class Custom_QLearner(rl_agent.AbstractAgent):
    """Tabular Q-Learning agent.
    ...
    def __init__(self,
        ...
        self._init_q_values('[0.0]', init_q_values)

    def _init_q_values(self, info_state, init_q_values):
        for action in range(self._num_actions):
            self._q_values[info_state][action] = init_q_values[action]
    ...
    def _boltzman(self, info_state, legal_actions, epsilon):
        temperature = 0.1
        ...
        for action in legal_actions:
            probs[action] = np.exp(self._q_values[info_state][action] / temperature) / sum(
                np.exp(np.array(list(self._q_values[info_state].values())) / temperature))
